...
{heatmap_html}
    ...
    <script>
        ...
        function loadDailyCalendar() {{
            ...
            var img = '<img class="daily-calendar-image" src="' + imgSrc + '" alt="单向历" onclick="loadDailyCalendar()" />';
            
            document.getElementById("dailyCalendarContent").innerHTML = img;
            
            // 监听图片加载错误
            var imgElement = document.querySelector('.daily-calendar-image');
            if (imgElement) {{
                imgElement.onerror = function() {{
                    document.getElementById("dailyCalendarContent").innerHTML = 
                        '<div class="daily-calendar-loading">今日单向历暂时无法加载</div>';
                }};
            }}
        }}
